...
landcover = ['urban', 'tree', 'shrub', 'moss', 'grass', 'bare', 'crops', 'forest', 'water-seasonal', 'water-permanent',
             'snow',]
...
for w in landcover:
    if not os.path.exists(StDev + '/' + w):
        os.mkdir(StDev + '/' + w)

for d in folders:
    tifList = [str(os.path.join(downloads, d, f)) for f in os.listdir(downloads + '/' + d) if
    ...
    for af in tifList:
        ...
        if 'StdDev' in af:
            for st in landcover:
                if st in af:
                    dest = os.path.join(StDev, st, os.path.basename(af))
                    #shutil.move(af, dest)
                    shutil.copy(af, dest)
